...
defineOptions({
    layout: useLayout(AppLayout, () => ({
        breadcrumbs: [
            ...
            {
                title: trans('pages.teams.expenses.sub_categories.create.title'),
                href: route('teams.expenses.sub-categories.create', {
                    team: route().params.team,
                    expenseType: route().params.expenseType,
                }),
            },
...
const form = useExpenseSubCategoryForm();

function submit() {
    const { team, expenseType } = props;
    form.post(route('teams.expenses.sub-categories.create', { team, expenseType }));
}
</script>

<template>
    <Head :title="$t('pages.teams.expenses.sub_categories.create.title')" />
    ...
    <Form :form @submit="submit()">
        <Section>
            <SectionHeader>
                ...
                <SectionDescription>
                    {{ $t('pages.teams.expenses.sub_categories.create.description') }}
                </SectionDescription>
            </SectionHeader>
